...
# plt.subploat() 함수를 사용하여 하나의 그림인 figure에 개별 서브플롯을 하나씩 추가
# 첫 번째 서브플롯 설정(선 그래프)
plt.subplot(2, 1, 1)
# 선 그래프 그리기
...
# x, y값 설정
x = parch_counts.indexastype(str) # 데이터프레임의 인덱스를 문자열로 변환
y1 = parch_counts[0].values # 사망자
y2 = parch_counts[1].values # 생존자
...
ax1.tick_params(axis='y', lavecolor='indigo') # 눈금의 모양과 속성
...
# 두 번째 그래프 설정 (막대 그래프)
ax2.bar(x, y2, color='deeppink', alpha=0.7, width=0.7, label='Not Survived')
ax
